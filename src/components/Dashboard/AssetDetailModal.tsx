...
interface AssetDetailModalProps {
    selectedAssetInfo: { title: string } | null;
...
const AssetDetailModal: React.FC<AssetDetailModalProps> = ({
    ...
    const getAssetTransactions = (assetTitle: string) => {
        ...
        return transactions
            .filter(t => t.method === assetType || (t as any).category === assetType || t.cardBank === assetType)
                        ...
                        getAssetTransactions(selectedAssetInfo.title).map((t, i) => (
                            <div key={i} className="flex justify-between items-center p-4 rounded-2xl bg-slate-800/50 border border-slate-700/50">
                                <div className="flex flex-col">
                                    <span className="text-xs font-bold text-white leading-none mb-1">{t.desc || 'İşlem'}</span>
