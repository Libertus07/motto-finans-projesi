...
const LiquidationModal: React.FC<LiquidationModalProps> = ({
    ...
    const handleConfirm = () => {
        if (!qty || !price || Number(qty) <= 0 || Number(price) <= 0) return alert("Geçerli değerler giriniz.");
        if (Number(qty) > investment.quantity) return alert("Stoktan fazla satış yapılamaz.");
