...
import { X, CheckCircle2, Plus, Coffee, Minus, FileText } from 'lucide-react';
...
interface ProductOptionsModalProps {
    ...
    onConfirm: (product: Product & { quantity: number; note: string; selectedOptions: any[] }) => void;
}

const ProductOptionsModal: React.FC<ProductOptionsModalProps> = ({ isOpen, onClose, product, onConfirm }) => {
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div className="mt-4">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                            <FileText size={12} /> Özel Not
                        </label>
                        <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Örn: Buzsuz olsun, Çok pişmiş..." className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white text-sm outline-none focus:border-indigo-500 transition-all resize-none h-20 placeholder:text-slate-600" />
                    </div>
                </div>
