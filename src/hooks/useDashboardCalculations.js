...
// Custom hook for all Dashboard calculations
export default function useDashboardCalculations(stats, transactions, customers, products, ingredients, tables) {
    // ========================================================================
    ...
    const starOfTheDay = useMemo(() => {
        ...
        const topPhone = Object.keys(todayPoints).reduce((a, b) =>
            todayPoints[a] > todayPoints[b] ? a : b, null);
        ...
        return {
            name: leader ? `${leader.name} ${leader.surname[0]}.` : "Gizli Kahraman",
    ...
    const topSpender = useMemo(() => {
        ...
        const pointRedeems = transactions
            ...
            .reduce((acc, curr) => {
                acc[curr.customerPhone] = (acc[curr.customerPhone] || 0) + (curr.pointsSpent || 0);
                return acc;
            }, {});

        const topPhone = Object.keys(pointRedeems).reduce((a, b) =>
            pointRedeems[a] > pointRedeems[b] ? a : b, null);
    ...
    const topLoyaltyProduct = useMemo(() => {
        ...
        const topName = Object.keys(productStats).reduce((a, b) =>
            productStats[a] > productStats[b] ? a : b, null);
    ...
    const topLoyaltyProductReal = useMemo(() => {
        const share = loyaltyAnalytics.totalPointsDistributed > 0
    ...
    const topProducts = products
        ...
        .map(p => ({
            ...
            share: totalSales > 0 ? Math.round((p.salesCount / totalSales) * 100) : 0
        }));
    ...
    // Stock value
    const estimatedStockValue = ingredients.reduce((sum, ing) => sum + (ing.price * (ing.stock || 0)), 0);
