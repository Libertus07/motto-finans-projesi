...
import { Product, Table, Transaction } from '../types';

// ✅ DÜZELTME: Gemini API Key artık .env'den VITE_ önekiyle çekiliyor.
export const GEMINI_API_KEY: string = import.meta.env.VITE_GEMINI_API_KEY || "API Anahtarı Yüklenemedi";
export const ADMIN_PASSWORD: string = import.meta.env.VITE_ADMIN_PASSWORD || "ADMIN_PASSWORD_NOT_SET";

// 👇 BURASI EKLENDİ: Tüm kullanıcıların ortak bağlanacağı dükkan kimliği
export const SHOP_ID: string = 'motto_coffee_sube_01';
...
export const CATEGORIES: string[] = ['Kahveler', 'Sıcak İçecekler', 'Tatlılar', 'Yemek', 'Diğer'];
export const COLORS: string[] = ['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6'];

export const COLOR_MAP: Record<string, { bg: string; text: string; hoverBorder: string }> = {
...
export const INITIAL_TRANSACTIONS: Partial<Transaction>[] = [
...
export const INITIAL_PRODUCTS: Partial<Product>[] = [
    { id: 'p1', name: 'Çay', price: 25, cost: 3.5, category: 'Sıcak İçecekler', sold: 1543, order: 1 } as any,
    { id: 'p2', name: 'Türk Kahvesi', price: 95, cost: 8.0, category: 'Kahveler', sold: 145, order: 2 } as any,
    { id: 'p3', name: 'Tiramisu', price: 180, cost: 101.0, category: 'Tatlılar', sold: 12, order: 3 } as any,
];
...
// 50 MASA
export const INITIAL_TABLES: Table[] = Array.from({ length: 50 }, (_, i) => {
    const num = i + 1;
    let zone = 'Bahçe';
