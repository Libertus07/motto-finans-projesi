// pages/Dashboard.jsx (ORTAK HAVUZ UYUMLU ✅)
        ...
        <div className="space-y-6 animate-in fade-in duration-700 pb-10">
            <div className="flex flex-col md:flex-row justify-between items-end bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-8 rounded-[2rem] border border-indigo-500/20 shadow-2xl relative overflow-hidden">
                <div className="relative z-10">
                    <div className="flex items-center gap-2 text-indigo-400 mb-1 text-sm font-bold uppercase tracking-widest"><Activity size={14} className="animate-pulse"/> {greetingText} PATRON</div>
                    <h1 className="text-4xl font-black text-white tracking-tight">Motto <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Yönetim Paneli</span></h1>
                    <p className="text-slate-400 mt-2 flex items-center gap-2"><Clock size={16}/> {new Date().toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', weekday: 'long' })}</p>
                </div>
                <div className="relative z-10 text-right mt-4 md:mt-0"><p className="text-slate-400 text-xs font-bold uppercase tracking-wider">TOPLAM NET VARLIK</p><h2 className="text-3xl font-black text-emerald-400 tracking-tight">{formatCurrency(totalNetWorth)} <span className="text-lg text-emerald-600">₺</span></h2></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="group bg-slate-800/60 backdrop-blur-xl p-5 rounded-2xl border border-slate-700 hover:border-purple-500 transition-all duration-300">
                    <div className="flex justify-between items-start"><div><p className="text-xs font-bold text-slate-400 uppercase">Ay Sonu Tahmini</p><h3 className="text-2xl font-extrabold text-white mt-1">{formatCurrency(calculateFutureCashflow)} ₺</h3></div><div className="p-3 bg-purple-500/20 rounded-xl text-purple-400"><Zap size={20}/></div></div>
                    <div className="mt-4 flex items-center gap-2 text-xs text-purple-400"><Target size={12}/> <span>AI Öngörüsü</span></div>
                </div>
            ...
            <div className="bg-slate-800/40 p-4 rounded-2xl border border-slate-700 flex items-center justify-between gap-6">
                <div className="flex items-center gap-4 min-w-fit">
                    ...
                    <div><p className="text-xs font-bold text-slate-400 uppercase">Anlık Doluluk</p><p className="text-white font-bold">{occupiedTables} / {totalTables} Masa</p></div>
                </div>
                <div className="flex-1">
                    <div className="flex justify-between mb-1 text-xs text-slate-400"><span>Doluluk Oranı</span><span className="text-amber-400 font-bold">% {occupancyRate.toFixed(0)}</span></div>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    <div className="bg-gradient-to-br from-emerald-900/40 to-slate-800 p-5 rounded-2xl border border-emerald-500/20 hover:border-emerald-500/50 transition-all">
                        <div className="flex justify-between items-start mb-4"><div className="p-2 bg-emerald-500/20 rounded-lg text-emerald-400"><Banknote size={20}/></div><span className="text-xs font-bold text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded">NAKİT</span></div>
                        <p className="text-slate-400 text-xs uppercase mb-1">Kasa Bakiyesi</p><h4 className="text-xl font-bold text-white">{formatCurrency(stats.assets.cash)} ₺</h4>
                    </div>
                    <div className="bg-gradient-to-br from-red-900/40 to-slate-800 p-5 rounded-2xl border border-red-500/20 hover:border-red-500/50 transition-all">
                        <div className="flex justify-between items-start mb-4"><div className="p-2 bg-red-500/20 rounded-lg text-red-400"><Landmark size={20}/></div><span className="text-xs font-bold text-red-500 bg-red-500/10 px-2 py-1 rounded">BANKA</span></div>
                        <p className="text-slate-400 text-xs uppercase mb-1">Ziraat Bankası</p><h4 className="text-xl font-bold text-white">{formatCurrency(stats.assets.ziraat)} ₺</h4>
                    </div>
                    <div className="bg-gradient-to-br from-blue-900/40 to-slate-800 p-5 rounded-2xl border border-blue-500/20 hover:border-blue-500/50 transition-all">
                        <div className="flex justify-between items-start mb-4"><div className="p-2 bg-blue-500/20 rounded-lg text-blue-400"><Building2 size={20}/></div><span className="text-xs font-bold text-blue-500 bg-blue-500/10 px-2 py-1 rounded">BANKA</span></div>
                        <p className="text-slate-400 text-xs uppercase mb-1">Halk Bankası</p><h4 className="text-xl font-bold text-white">{formatCurrency(stats.assets.halk)} ₺</h4>
                    </div>
                    <div className="bg-gradient-to-br from-purple-900/40 to-slate-800 p-5 rounded-2xl border border-purple-500/20 hover:border-purple-500/50 transition-all">
                        <div className="flex justify-between items-start mb-4"><div className="p-2 bg-purple-500/20 rounded-lg text-purple-400"><CreditCard size={20}/></div><span className="text-xs font-bold text-purple-500 bg-purple-500/10 px-2 py-1 rounded">DİĞER</span></div>
                        <p className="text-slate-400 text-xs uppercase mb-1">Diğer Hesaplar</p><h4 className="text-xl font-bold text-white">{formatCurrency(stats.assets.iban)} ₺</h4>
                    </div>
                    <div className="bg-gradient-to-br from-amber-900/40 to-slate-800 p-5 rounded-2xl border border-amber-500/20 hover:border-amber-500/50 transition-all">
                        <div className="flex justify-between items-start mb-4"><div className="p-2 bg-amber-500/20 rounded-lg text-amber-400"><Coins size={20}/></div><span className="text-xs font-bold text-amber-500 bg-amber-500/10 px-2 py-1 rounded">YATIRIM</span></div>
                        <p className="text-slate-400 text-xs uppercase mb-1">Altın / Döviz</p><h4 className="text-xl font-bold text-white">{formatCurrency(stats.investmentStats.currentValue)} ₺</h4>
                    </div>
                </div>
            </div>
                    ...
                    <div className="flex-1 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData}>
                                <defs><linearGradient id="colorInc" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6366f1" stopOpacity={0.4}/><stop offset="95%" stopColor="#6366f1" stopOpacity={0}/></linearGradient></defs>
                ...
                <div className="flex flex-col gap-6">
                    <div className="bg-slate-800 p-6 rounded-3xl border border-slate-700 shadow-xl relative overflow-hidden flex flex-col items-center justify-center h-[200px]">
                        ...
                        <div className="w-full h-full relative">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center pt-6"><span className="text-xs text-slate-400 mb-1">PROGRES</span><span className="text-2xl font-black text-white tracking-tight">{formatCurrency(stats.monthlyIncome)}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 bg-slate-800 p-5 rounded-3xl border border-slate-700 shadow-xl overflow-hidden flex flex-col h-[180px]">
                        <div className="flex justify-between items-center mb-3"><h4 className="text-sm font-bold text-slate-400 uppercase flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></div> Akış</h4></div>
                        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pr-2">
                            {recentSales.map((t, idx) => (
                                <div key={t.id || idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-900/50 border border-slate-700/50 hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <span className={`text-xs font-bold ${t.type === 'income' ? 'text-emerald-400' : 'text-red-400'}`}>{t.type === 'income' ? '+' : '-'}{formatCurrency(t.amount)}</span>
                                </div>
                            ))}
                        </div>
                    </div>
