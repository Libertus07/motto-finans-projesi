// pages/CashierPOS.jsx (ADİSYON EKLENMİŞ SON HALİ)
...
import Receipt from '../components/Receipt'; // 👇 Receipt Bileşeni Import Edildi
...
const CashierPOS = ({ products, ingredients }) => {
    ...
    // 👇 YENİ FONKSİYON: Fiş Yazdırma
    const handlePrintReceipt = () => {
        if (cart.length === 0) return;

        setPrintData({
            title: 'Hızlı Satış Fişi',
            type: 'Satış Fişi',
            date: new Date().toLocaleString('tr-TR'),
            // Sepetteki ürünleri Receipt formatına dönüştürüyoruz
            items: cart.map(item => ({
                name: item.name,
        ...
        // State güncellendikten hemen sonra yazdırma penceresini aç
    ...
    const handleCheckout = async () => {
        ...
        const transactionData = {
            ...
            type: 'income',
            amount: totalAmount,
            desc: `POS Satış (${cart.length} Kalem)`,
            method: transMethod,
        ...
        try {
            ...
            // İsterseniz satış tamamlanınca otomatik fiş yazdırmak için burayı açabilirsiniz:
            // handlePrintReceipt(); 
    ...
    return (
        <div className="flex flex-col lg:flex-row h-[calc(100vh-100px)] gap-6 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden shadow-xl">
                <div className="p-4 border-b border-slate-700 space-y-4 bg-slate-800/50">
                    ...
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                        {categories.map(cat => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-colors ${selectedCategory === cat ? 'bg-indigo-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>{cat}</button>
                        ))}
                    </div>
                </div>
            ...
            {/* SAĞ: SEPET VE ÖDEME */}
            <div className="w-full lg:w-[400px] bg-slate-800 rounded-2xl border border-slate-700 flex flex-col shadow-2xl shrink-0">
                ...
                <div className="p-5 bg-slate-900 border-t border-slate-800 space-y-4">
                    <div className="flex justify-between items-end">
                        <span className="text-slate-400 text-sm font-medium">Toplam Tutar</span>
                        <span className="text-3xl font-black text-white tracking-tight">{formatCurrency(totalAmount)} <span className="text-lg text-slate-600 font-medium">₺</span></span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <button onClick={() => setPaymentMethod('card')} className={`py-3 rounded-xl font-bold flex flex-col items-center justify-center gap-1 transition-all border-2 ${paymentMethod === 'card' ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-900/20' : 'bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-600'}`}>
                            <CreditCard size={24}/> <span className="text-xs">KART</span>
                        </button>
                    </div>
                    ...
                    {paymentMethod === 'card' && (
                        <div className="grid grid-cols-3 gap-2 animate-in slide-in-from-top-2 fade-in">
                            {bankOptions.map(option => (
                                <button key={option.key} onClick={() => setCardBank(option.key)} className={`py-2 rounded-lg text-[10px] font-bold transition-colors border ${cardBank === option.key ? 'bg-purple-600 border-purple-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}>
                                    {option.label}
                                </button>
                            ))}
                        </div>
                    ...
                    <button 
                        ...
                        className="w-full py-3 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors border border-slate-600"
                    >
                    ...
                    <button 
                        ...
                        className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed ${successMsg ? 'bg-emerald-500 text-white' : (paymentMethod === 'cash' ? 'bg-emerald-600 hover:bg-emerald-500 text-white' : 'bg-blue-600 hover:bg-blue-500 text-white')}`}
                    >
                        {successMsg ? <><CheckCircle2/> {successMsg}</> : 'TAHSİL ET'}
                    </button>
                </div>
                
                {/* 👇 GİZLİ FİŞ BİLEŞENİ (Sadece print komutuyla çalışır) */}
