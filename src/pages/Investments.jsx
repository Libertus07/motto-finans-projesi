...
const Investments = ({ investments, marketRates }) => {
    ...
    const handleLiquidateConfirm = async (sellQty, sellPrice) => {
                ...
                <div className="lg:col-span-2 bg-slate-800 p-6 rounded-2xl border border-slate-700">
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="text-xs text-slate-500 block mb-1">Tarih</label><input type="date" value={newInvestment.date} onChange={(e) => setNewInvestment({...newInvestment, date: e.target.value})} className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white outline-none focus:border-emerald-500 transition-all"/></div>
                            <div><label className="text-xs text-slate-500 block mb-1">Varlık Türü</label><select value={newInvestment.type} onChange={(e) => setNewInvestment({...newInvestment, type: e.target.value, currentPrice: marketRates[e.target.value] || ''})} className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white outline-none focus:border-emerald-500 transition-all">{Object.keys(INITIAL_MARKET_RATES).map(rate => <option key={rate} value={rate}>{rate}</option>)}</select></div>
                        </div>
                        <div className="grid grid-cols-3 gap-3">
                            <div><label className="text-xs text-slate-500 block mb-1">Adet/Miktar</label><input type="number" placeholder="0" value={newInvestment.quantity} onChange={(e) => setNewInvestment({...newInvestment, quantity: e.target.value})} className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white outline-none focus:border-emerald-500"/></div>
                            <div><label className="text-xs text-slate-500 block mb-1">Alış Fiyatı (Birim)</label><input type="number" placeholder="0" value={newInvestment.buyPrice} onChange={(e) => setNewInvestment({...newInvestment, buyPrice: e.target.value})} className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white outline-none focus:border-emerald-500"/></div>
                            <div><label className="text-xs text-yellow-500 block mb-1 font-bold">Güncel Fiyat (Birim)</label><input type="number" placeholder="0" value={newInvestment.currentPrice} onChange={(e) => setNewInvestment({...newInvestment, currentPrice: e.target.value})} className="w-full bg-slate-900 border border-yellow-600/50 rounded-lg p-3 text-yellow-400 font-bold outline-none focus:border-yellow-500"/></div>
                        </div>
                        ...
                        <tbody className="divide-y divide-slate-700">{investments.map(inv => { 
                                ...
                                return (
                                    <tr key={inv.id} className="hover:bg-slate-700/50 group transition-colors">
                                        ...
                                        <td className="px-4 py-3 font-bold text-white flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-yellow-500"></span> {inv.type}</td>
                                        <td className="px-4 py-3 font-mono text-slate-300">{inv.quantity}</td>
                                        <td className="px-4 py-3">{formatCurrency(inv.buyPrice)} ₺</td>
                                        <td className="px-4 py-3"><input type="number" value={currentPrice} onChange={(e) => handleUpdateInvestment(inv.id, 'currentPrice', e.target.value)} className="bg-slate-900 border border-slate-600 rounded px-2 py-1 text-yellow-400 font-bold w-24 text-right outline-none focus:border-yellow-500 transition-all text-xs"/></td>
                                        <td className="px-4 py-3 font-bold text-white">{formatCurrency(totalVal)} ₺</td>
                                        <td className={`px-4 py-3 font-bold ${profit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{profit >= 0 ? '+' : ''}{formatCurrency(profit)} ₺</td>
                                        <td className="px-4 py-3 text-right flex items-center justify-end gap-2">
                                            <button onClick={() => setLiquidationData(inv)} className="text-xs bg-indigo-900/50 text-indigo-300 px-3 py-1.5 rounded-lg hover:bg-indigo-600 hover:text-white transition-colors border border-indigo-500/30">Bozdur</button>
                                            <button onClick={() => setDeleteId(inv.id)} className="text-slate-500 hover:text-red-500 p-1.5 hover:bg-slate-700 rounded transition-colors"><Trash2 size={16}/></button>
                                        </td>
                                    </tr>
                                ) 
                            })}
