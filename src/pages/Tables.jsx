// pages/Tables.jsx (GÜNCELLENMİŞ TAM İÇERİK)
...
const Tables = ({ tables, products }) => {
    ...
    const [isEmptyConfirmOpen, setIsEmptyConfirmOpen] = useState(false); // Temizleme onayı
    const [isCloseModalOpen, setIsCloseModalOpen] = useState(false);   // Hesap kapatma onayı
    ...
    // 👇 Masayı Temizle İşlemi (Modal Onaylı)
    ...
    // 👇 Hesap Kapatma İşlemi (Modal Onaylı)
    ...
    return (
        <div className="flex h-[calc(100vh-100px)] overflow-hidden">
            ...
            <ConfirmationModal
                ...
                onConfirm={confirmMarkAsEmpty}
                title="Masayı Temizle"
                message="Bu masayı boş ve temiz olarak işaretlemek istediğinize emin misiniz? (Siparişler silinir)"
                type="warning"
                confirmText="TEMİZLE"
            />
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4">
                    {filteredTables.map(table => (
                        <button key={table.id} onClick={() => setSelectedTable(table)} className={`flex flex-col items-center justify-center p-6 rounded-2xl shadow-xl transition-all h-32 ${getTableStatusColor(table.status)} text-white relative group`}>
            ...
            ) : <div className={`${THEME.card} w-full md:w-96 shrink-0 border-l ${THEME.border} flex flex-col items-center justify-center text-slate-600`}><TableIcon size={48} className="mb-4"/><p>Detayları görmek için sol taraftan bir masa seçin.</p></div>}
        </div>
    );
};
