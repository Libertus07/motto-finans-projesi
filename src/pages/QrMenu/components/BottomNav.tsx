...
import { Landmark, Search, ShoppingBag, Bell, User, LogIn, Sparkles } from 'lucide-react';
...
const BottomNav: React.FC<BottomNavProps> = ({ view, setView, cartCount, onOpenService, onResetCategory, t, isMember, onAuthClick, onAccountClick }) => {
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-[#FDFBF7] border-t-2 border-[#432818]/5 shadow-[0_-10px_40px_rgba(67,40,24,0.05)] z-40 pb-safe">
            <div className="flex justify-between items-end h-20 px-6 max-w-md mx-auto relative">
                ...
                <button
                    ...
                    className={`flex flex-col items-center justify-center w-14 h-14 mb-2 transition-colors ${view === 'account' ? 'text-[#432818]' : 'text-[#432818]/40'}`}
                >
                    {isMember ? <User size={24} strokeWidth={view === 'account' ? 2.5 : 2} /> : <LogIn size={24} strokeWidth={2} />}
                    <span className={`text-[9px] mt-1 font-cinzel ${view === 'account' ? 'font-bold' : 'font-medium'}`}>
                        {isMember ? t('account') : t('login')}
                    </span>
                </button>
            </div>
        </div>
