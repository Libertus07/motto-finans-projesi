...
import {
    ...
    ShoppingBag, Package, Heart, Clock, LucideIcon
} from 'lucide-react';
...
const CustomerLoyalty = () => {
        ...
        if (user?.id) {
            const unsub = onSnapshot(doc(db, customerPath, user.id), (snap) => {
                if (snap.exists()) {
                    ...
                    setUser({ id: snap.id, ...data } as LoyaltyUser);
    ...
    const handleCheckPhone = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!snap.empty) {
                const d = snap.docs[0];
                setUser({ id: d.id, ...d.data() } as LoyaltyUser);
                setStep('dashboard');
            } else {
                setStep('register');
            }
            ...
            <div className="max-w-2xl mx-auto px-6 mt-8 space-y-8 relative z-10">
                ...
                <div className="flex p-2 bg-white/5 border border-white/10 rounded-[28px]">
                    <button
                        ...
                        className={`flex-1 py-4 text-xs font-black rounded-[24px] transition-all ${activeTab === 'home'
                    ...
                    <button
                        ...
                        className={`flex-1 py-4 text-xs font-black rounded-[24px] transition-all ${activeTab === 'history'
                                ? 'bg-white text-black shadow-xl'
                                : 'text-slate-400 hover:text-slate-200'
                            }`}
                    >
                            ...
                            {user?.history?.slice().reverse().map((h, i) => (
                                ...
                                >
                                    <div className="flex items-center gap-5">
                                        <div className={`w-14 h-14 rounded-[22px] flex items-center justify-center ${h.points > 0
                                    ...
                                    <span className={`text-2xl font-black ${h.points > 0 ? 'text-emerald-400' : 'text-rose-400'
                                        }`}>
                                        {h.points > 0 ? '+' : ''}{h.points}
                                    </span>
                                </div>
