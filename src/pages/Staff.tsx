// src/pages/Staff.tsx (ORTAK HAVUZ ENTEGRASYONU ✅)

import React, { useState } from 'react';
import { Users, UserPlus, DollarSign, Wallet, Trash2, Save, X, CreditCard, Banknote, Calendar, CheckCircle2, KeyRound, Edit, Dices } from 'lucide-react';
...
const Staff: React.FC = () => {
    ...
    const [salaryModal, setSalaryModal] = useState({ open: false, staffId: null as string | null, staffName: '', baseSalary: 0, deduction: 0, netSalary: 0 });
    const [deleteId, setDeleteId] = useState<string | null>(null);
    const [historyDelete, setHistoryDelete] = useState<{ staffId: string; item: StaffFinanceItem; type: string } | null>(null);
    const [infoModal, setInfoModal] = useState<{ isOpen: boolean; type: 'success' | 'warning' | 'error'; title: string; message: string }>({ isOpen: false, type: 'success', title: '', message: '' });
    const [editId, setEditId] = useState<string | null>(null); // ✨ Düzenleme modu için ID

    const [newStaff, setNewStaff] = useState<NewStaffState>({ name: '', role: 'Garson', salary: '', startDate: '', salaryDay: '', pin: '', advances: [], payments: [] });
    const [amount, setAmount] = useState('');
    const [paymentMethod, setPaymentMethod] = useState<'cash' | 'ziraat' | 'halk' | 'iban'>('cash');
    ...
    const openSalaryModal = (person: StaffType) => {
        ...
        setAmount((net > 0 ? net : 0).toString());
    ...
    const handleSaveStaff = async () => {
        if (!newStaff.name || !newStaff.salary || !newStaff.salaryDay || !newStaff.pin) {
        ...
        try {
            if (editId) {
                // ✨ GÜNCELLEME İŞLEMİ
                await updateDoc(doc(db, 'artifacts', appId, 'shops', CURRENT_SHOP_ID, 'staff', editId), {
                    ...newStaff, salary: Number(newStaff.salary), salaryDay: Number(newStaff.salaryDay)
                });
                setInfoModal({ isOpen: true, type: 'success', title: 'Güncellendi', message: 'Personel bilgileri güncellendi.' });
            } else {
                // ✨ YENİ EKLEME İŞLEMİ
            ...
            setNewStaff({ name: '', role: 'Garson', salary: '', startDate: '', salaryDay: '', pin: '', advances: [], payments: [] });
            setEditId(null);
            setIsFormOpen(false);
        } catch (error) { setInfoModal({ isOpen: true, type: 'error', title: 'Hata', message: 'Kaydedilemedi.' }); }
    };

    const handleEditClick = (person: StaffType) => {
        setNewStaff({
            name: person.name,
            role: person.role,
            salary: person.salary,
            startDate: person.startDate || '',
            salaryDay: person.salaryDay,
            pin: person.pin || '',
    ...
    const handleGiveAdvance = async () => {
        if (!amount || Number(amount) <= 0 || !advanceModal.staffId) return;
        try {
    ...
    const handlePaySalary = async () => {
        if (!amount || Number(amount) <= 0 || !salaryModal.staffId) return;
        try {
            const transMethod = paymentMethod === 'cash' ? 'cash' : 'card';
            const cardBank = paymentMethod === 'cash' ? null : paymentMethod;
    ...
    const confirmHistoryDelete = async () => {
        ...
        try {
            ...
            const currentArray = (currentStaff as any)[field] || []; // Use any cast for dynamic access if needed, or stricter type check
            const newArray = currentArray.filter((i: StaffFinanceItem) => i.id !== item.id);
            ...
            // 2. KASADAN İŞLEMi SİL (PARAYI İADE ET)
    ...
    // ✨ YENİ: Personel İstatistiklerini Hesapla
    const getStaffStats = (personId: string) => {
        const today = new Date().toISOString().split('T')[0];
        // Bugün bu personelin yaptığı satışlar (income)
        const todaySales = transactions
            .filter(t => t.type === 'income' && t.staffId === personId && t.date === today)
            .reduce((sum, t) => sum + Number(t.amount), 0);

        return { todaySales };
    };

    // ✨ YENİ: Online Kontrolü (Son 2 dakika içinde aktif mi?)
    const isStaffOnline = (lastSeen?: string) => {
        if (!lastSeen) return false;
        const diff = new Date().getTime() - new Date(lastSeen).getTime();
        return diff < 2 * 60 * 1000; // 2 dakika
    };

    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
            <ConfirmationModal isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={confirmDeleteStaff} title="Personeli Sil" message="Emin misiniz?" type="danger" loading={false} />
            <ConfirmationModal isOpen={!!historyDelete} onClose={() => setHistoryDelete(null)} onConfirm={confirmHistoryDelete} title="İşlemi İptal Et" message="Bu ödemeyi silmek istediğinize emin misiniz? Tutar kasaya geri eklenecektir." type="warning" confirmText="Evet, İptal Et" loading={false} />
            ...
            <div className="flex justify-between items-end border-b border-slate-800 pb-6">
                ...
                }} className={`px-5 py-3 rounded-xl font-bold flex items-center gap-2 transition-all ${isFormOpen ? 'bg-slate-700 text-slate-300' : 'bg-indigo-600 text-white'}`}>{isFormOpen ? <X size={20} /> : <UserPlus size={20} />} {isFormOpen ? 'Vazgeç' : 'Personel Ekle'}</button>
            </div>
            ...
            {isFormOpen && (
                <div className="bg-slate-800/50 p-6 rounded-2xl border border-indigo-500/30 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
                        ...
                        <div>
                            <label className="text-xs text-slate-400 font-bold block mb-1">GİRİŞ PİNİ</label>
                            <div className="flex gap-2"><input type="text" maxLength={6} placeholder="6 Haneli" value={newStaff.pin} onChange={(e) => setNewStaff({ ...newStaff, pin: e.target.value.replace(/\D/g, '') })} className="w-full bg-slate-900 border border-slate-600 rounded-xl p-3 text-white outline-none focus:border-indigo-500 text-center font-mono tracking-widest font-bold" /><button onClick={generateRandomPin} className="p-3 bg-slate-700 hover:bg-slate-600 text-white rounded-xl transition-colors" title="Rastgele PIN"><Dices size={20} /></button></div>
                        </div>
                        <button onClick={handleSaveStaff} className="bg-emerald-600 hover:bg-emerald-500 text-white h-[48px] rounded-xl font-bold lg:col-span-6 mt-2 flex items-center justify-center gap-2"><Save size={18} /> {editId ? 'Güncelle' : 'Kaydet'}</button>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {staff.map((person) => {
                    ...
                    const history = [
                        ...(person.advances || []).map(a => ({ ...a, type: 'advance' as const, label: 'Avans', color: 'text-red-400' })),
                        ...(person.payments || []).map(p => ({ ...p, type: 'salary' as const, label: 'Maaş', color: 'text-emerald-400' }))
                    ].sort((a, b) => b.id - a.id).slice(0, 5);

                    const stats = getStaffStats(person.id);
                    const isOnline = isStaffOnline(person.lastSeen);
                    ...
                    return (
                        <div key={person.id} className="bg-slate-800 rounded-2xl border border-slate-700 hover:border-indigo-500/50 p-6 shadow-xl relative group">
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-4">
                                    <div className="relative">
                                        ...
                                        {/* ✨ Online Göstergesi */}
                                        {isOnline && <div className="absolute bottom-0 right-0 w-4 h-4 bg-emerald-500 border-2 border-slate-800 rounded-full animate-pulse" title="Çevrimiçi"></div>}
                                    </div>
                                    <div>
                                        <h3 className="text-lg font-bold text-white">{person.name}</h3>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="bg-slate-700 text-slate-300 text-[10px] px-2 py-0.5 rounded font-bold uppercase">{person.role}</span>
                                            <div className="flex items-center gap-1 bg-slate-900/50 px-2 py-0.5 rounded border border-slate-700/50"><KeyRound size={10} className="text-slate-500" /><span className="text-[10px] font-mono text-slate-400 tracking-widest">{person.pin || '---'}</span></div>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => handleEditClick(person)} className="text-slate-600 hover:text-blue-400 transition-colors" title="Düzenle"><Edit size={18} /></button>
                                    <button onClick={() => setDeleteId(person.id)} className="text-slate-600 hover:text-red-400 transition-colors" title="Sil"><Trash2 size={18} /></button>
                                </div>
                            </div>
                            <div className="grid grid-cols-3 gap-2 mb-6 text-center">
                                ...
                                {/* ✨ Bugün Satış */}
                                <div className="bg-slate-900/50 p-2 rounded-xl border border-slate-700/50"><div className="text-[9px] text-slate-400 font-bold mb-1">Bugün Satış</div><div className="text-xs font-bold text-emerald-400">{formatCurrency(stats.todaySales)} ₺</div></div>
                                    ...
                                    <div className="space-y-1 max-h-32 overflow-y-auto custom-scrollbar pr-1">
                                        {history.map((item, idx) => (
                                            <div key={idx} className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-700/20 hover:bg-slate-700/40 group/item transition-colors">
                                                <div className="flex items-center gap-2"><span className="text-slate-400 font-medium">{new Date(item.date).toLocaleDateString('tr-TR')}</span><span className={`text-[9px] uppercase px-1.5 rounded bg-slate-800 ${item.type === 'advance' ? 'text-red-400' : 'text-emerald-400'}`}>{item.type === 'advance' ? 'Avans' : 'Maaş'}</span></div>
                                                <div className="flex items-center gap-2"><span className={`${item.type === 'advance' ? 'text-red-400' : 'text-emerald-400'} font-bold font-mono`}>{formatCurrency(item.amount)}</span><button onClick={() => setHistoryDelete({ staffId: person.id, item: item, type: item.type || 'advance' })} className="text-slate-600 hover:text-red-400 opacity-0 group-hover/item:opacity-100 transition-opacity" title="İptal Et"><X size={14} /></button></div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
            ...
            {advanceModal.open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-2xl w-full max-w-sm relative">
                        <button onClick={() => setAdvanceModal({ open: false, staffId: null, staffName: '' })} className="absolute top-4 right-4 text-slate-500 hover:text-white"><X size={20} /></button>
                        ...
                        <div className="space-y-4">
                            ...
                            {paymentMethod !== 'cash' && (<div className="grid grid-cols-3 gap-2">{['ziraat', 'halk', 'iban'].map(bank => (<button key={bank} onClick={() => setPaymentMethod(bank as any)} className={`py-2 text-xs font-bold rounded-lg border ${paymentMethod === bank ? 'bg-indigo-500/20 border-indigo-500 text-indigo-300' : 'bg-slate-900 border-slate-700 text-slate-500'}`}>{bank.toUpperCase()}</button>))}</div>)}
            ...
            {salaryModal.open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-2xl w-full max-w-sm relative">
                        <button onClick={() => setSalaryModal({ open: false, staffId: null, staffName: '', baseSalary: 0, deduction: 0, netSalary: 0 })} className="absolute top-4 right-4 text-slate-500 hover:text-white"><X size={20} /></button>
